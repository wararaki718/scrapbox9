...
class MatryoshkaEncoder(nn.Module):
    def __init__(self, vocabulary_size: int, embedding_dim: int, num_heads: int, num_layers: int, max_length: int) -> None:
        super().__init__()
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, dimension: int | None = None) -> torch.Tensor:
        ...
        hidden = self.encoder(self.token_embedding(input_ids) + self.position_embedding(positions), src_key_padding_mask=attention_mask == 0)
